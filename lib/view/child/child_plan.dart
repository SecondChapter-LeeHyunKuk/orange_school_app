...
class _ChildPlan extends State<ChildPlan> {
  ...
  // void showScheduleInfo(int scheduleId){
  //
  ...
  Future<void> getMonthly() async {
    SharedPreferences pref = await SharedPreferences.getInstance();
    var response = await apiRequestGet(urlMonthly,  { "commonMemberIdList" : children[selectedChildIndex]["id"].toString(), "monthStartDate" : DateFormat('yyyy-MM-dd').format(selectMonth())});
  ...
  Future<void> getDaily() async {
    SharedPreferences pref = await SharedPreferences.getInstance();
    var response = await apiRequestGet(urlDaily,  {"commonMemberIdList" : children[selectedChildIndex]["id"].toString(), "dayDate" :DateFormat('yyyy-MM-dd').format(selectDay)});
    var body =jsonDecode(utf8.decode(response.bodyBytes));
    if(response.statusCode == 200){
      List allDay = [];
      List notAllDay = [];
  ...
  Future<void> getWeek() async {
    ...
    SharedPreferences pref = await SharedPreferences.getInstance();
    var response = await apiRequestGet(urlWeek,  { "commonMemberIdList" : children[selectedChildIndex]["id"].toString(), "weekStartDate" :DateFormat('yyyy-MM-dd').format(selectDay)});
    var body =jsonDecode(utf8.decode(response.bodyBytes));
    if(response.statusCode == 200){
      List temp = [];
