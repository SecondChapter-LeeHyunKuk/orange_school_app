...
class _SearchSchoolBottom extends State<SearchSchoolBottom> {
  ...
  Future<Response> searchSchool() async {
    ...
    var responseResult = await apiRequestGet(url,{"Type" : "json", "pIndex" : 1.toString(), "pSize": 20.toString(), "SCHUL_NM" : nowKeyword, "key" : schoolKey});
  ...
  void scroll() async {
    var responseResult =  await apiRequestGet(url,{"Type" : "json", "pIndex" : (index + 1).toString(), "pSize": 20.toString(), "SCHUL_NM" : nowKeyword , "key" : schoolKey});
    var response =jsonDecode(utf8.decode(responseResult.bodyBytes));
