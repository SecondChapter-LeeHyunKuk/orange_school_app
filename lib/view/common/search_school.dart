...
class _SearchSchool extends State<SearchSchool> {
  ...
  void searchSchool() async {
    index = 1;
    data = [];
    var keyword = textEditingController.text.replaceAll(' ', '');
    var responseResult = await apiRequestGet(context, url,{"Type" : "json", "pIndex" : 1.toString(), "pSize": 10.toString(), "SCHUL_NM" : keyword , "key" : "8a3319b07614480db9a06c4906426c52"});
  ...
  void scroll() async {
    var keyword = textEditingController.text.replaceAll(' ', '');
    var responseResult =  await apiRequestGet(context, url,{"Type" : "json", "pIndex" : (index + 1).toString(), "pSize": 10.toString(), "SCHUL_NM" : keyword , "key" : "8a3319b07614480db9a06c4906426c52"});
    var response =jsonDecode(utf8.decode(responseResult.bodyBytes));
    if(response["schoolInfo"][0]["head"][1]["RESULT"]["CODE"] == "INFO-000"){
